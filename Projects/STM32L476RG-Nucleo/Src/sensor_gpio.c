...
void vcc_sw_gpio_init(void)
{
	...
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(GPIOB, &GPIO_InitStruct); 
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_11, GPIO_PIN_SET);//	断电	
	//HAL_GPIO_WritePin(GPIOB, GPIO_PIN_11, GPIO_PIN_RESET);//上电	
...
//	msg_gpio_init();
	lora_as62t30_gpio_init();//现在为电源管理引脚
	power_manage_gpio_init();//持续供电
