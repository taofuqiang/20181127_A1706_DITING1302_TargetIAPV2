...
static void TimerSetTimeout(TimerEvent_t *obj )
{
    ...
    if(obj->TimeCount  <  TIMER_MIN_TIMEOUT)
    {
        obj->TimeCount = TIMER_MIN_TIMEOUT;
    }
    ...
    while (1)
    {
        ...
        //必须保证成功 否则丢失
        if (stm32_lptim_cmp_en(TimerStampLast + obj->TimeCount))
        {
            break;
        }
    }
...
void TimerStart( TimerEvent_t *obj )
{
    ...
    else 
    {
        ...
        timeout = obj->TimeCount + elapsedTime;

        if(timeout < TimerListHead->TimeCount )
        {
            //插入到前面的需要把流逝的时间都减去
            DeltaContext = TimerElapsedTime_Get(); //intentionnal wrap around
